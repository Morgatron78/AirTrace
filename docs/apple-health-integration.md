...
- One aggregate row on Stats — "Avg sleep efficiency," 30-night average —
  the opposite framing from Trends' Sleep architecture chart: percent of
  time in bed *actually asleep* (Core+Deep+REM against
  Core+Deep+REM+**Awake**), so unlike the chart, Awake belongs in the
  denominator here on purpose. Omitted entirely (not a 0%) when zero
  nights in range have Watch data, same quiet-omission convention as
...
- `src/components/AllTimeTrendCard.jsx` / `src/components/charts/AllTimeLineChart.jsx` — remove the `weightReadings`/`heightCm`/`weightUnit` props and every `hasWeight`/`canShowBmi`-gated branch (the weight tile, the Weight/BMI `Segmented` toggle, the weight/BMI panel, `formatWeightKg`/`bmiFromWeightKg` imports). What remains is the AHI-only card and panel, which have nothing to do with Apple Health — the confound-shading mechanism in particular is built entirely from `STR.edf`'s own `setPressure`/`mode` fields, not Apple Health data.
- `src/screens/SettingsScreen.jsx` — remove the `weightUnit`/`onChangeWeightUnit` props, the `hasWeightData` state/fetch effect, and the "Weight unit" `Segmented` row. The height field itself (`heightUnit`/`onChangeHeightUnit`, the "Height" `Segmented` + `StepperRow`) is not Apple-Health code and has a real non-Apple-Health justification (BMI's own formula needs it) — but note it becomes dead UI with no consumer once weight/BMI is stripped, so it's reasonable to remove alongside this pass even though it isn't itself marked `// APPLE-HEALTH:`.
- `src/App.jsx` — remove the `weightUnit`/`onChangeWeightUnit` state, meta load, and the props threaded to `SettingsScreen`/`TrendsScreen`. Same dead-UI note as above applies to `heightUnit`/`profile.heightCm` if height is removed alongside weight.
