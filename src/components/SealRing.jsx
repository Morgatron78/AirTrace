    ...
    <div>
      <button onClick={() => setShowLegend((o) => !o)} style={{ position: 'relative', width: size, height: size, margin: '0 auto', display: 'block' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          {/* % rendered smaller than the digits — at 100% a full-size (or
              even half-size) glyph still pushed the text wide enough to
              overflow the ring. */}
          <span className="font-display" style={{ fontSize: size * 0.28, fontWeight: 800, color: T.ink, lineHeight: 1 }}>{goodPct}<span style={{ fontSize: '0.35em' }}>%</span></span>
